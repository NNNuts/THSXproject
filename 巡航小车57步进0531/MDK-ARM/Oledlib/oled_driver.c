...
{		
	...
	void I2C_Configuration(void)
	{
		...
		GPIO_InitStructure.GPIO_Pin =  IIC_SCL_Pin_X | IIC_SDA =Pin_X;
...
void OLED_Init(void)
{
	...
	WriteCmd(0x8D); //--set contrast control register
	WriteCmd(0x14); //亮度调节 0x00~0xff
	WriteCmd(0x20); //--set segment re-map 0 to 127
	WriteCmd(0x02); //--set normal display
	WriteCmd(0xA1); //--set multiplex ratio(1 to 64)
	WriteCmd(0xC8); //
	WriteCmd(0xDA); //0xa4,Output follows RAM content;0xa5,Output ignores RAM content
	WriteCmd(0x02); //-set display offset
	WriteCmd(0x81); //-not offset
	WriteCmd(0xEF); //--set display clock divide ratio/oscillator frequency
	WriteCmd(0xD9); //--set divide ratio
	WriteCmd(0xf1); //--set pre-charge period
	WriteCmd(0xDB); //
	WriteCmd(0x30); //--set com pins hardware configuration
	WriteCmd(0xA4);
	WriteCmd(0xA6); //--set vcomh
	WriteCmd(0xAF); //0x20,0.77xVcc
